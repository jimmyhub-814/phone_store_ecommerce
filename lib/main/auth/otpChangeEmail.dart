...
class OtpChangeEmailPage extends StatefulWidget {
  final String newEmail;
  final String password;
  static const routeName = '/otpChangeEmail';
  const OtpChangeEmailPage(
      {super.key, required this.newEmail, required this.password});

  @override
  State<OtpChangeEmailPage> createState() => _OtpChangeEmailPageState();
}

enum OtpStatus { resetPassword, changeEmail }

class _OtpChangeEmailPageState extends State<OtpChangeEmailPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (widget.newEmail.isNotEmpty) {
        await sendOtpEmail(widget.newEmail);
      }
    });
  }
  ...
  Future<void> onVerified() async {
    try {
      final newEmail = widget.newEmail;
      final oldPass = widget.password;
  ...
  Future<void> _onResendPressed() async {
    if (widget.newEmail.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Không có email để gửi lại OTP.',
            ),
          ),
        );
      }
    ...
    final ok = await sendOtpEmail(widget.newEmail);
    if (!ok && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Gửi lại OTP thất bại — thử lại sau.',
          ),
        ),
      );
    }
